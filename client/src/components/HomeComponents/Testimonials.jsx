  ...
  return (
    <section className="mt-10 py-16 px-6  text-gray-100" id="testimonials">
      <div className="max-w-6xl mx-auto text-center">
        <HeadingText
          hTextOne="The"
          hTextTwo="Testimonials"
          paragraph="What Clients Say"
        />
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t, index) => (
            <div
              key={index}
              className="relative bg-accentColor/20 dark:bg-gray-800 p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow text-left"
            >
              {/* Quote + Stars in same line */}
              <div className="flex items-center justify-between mb-4">
                <Quote className="w-6 h-6 text-accentColor mr-2" />
                <div className="flex">
                  {Array.from({ length: 5 }, (_, i) => (
                    <Star
                      ...
                      className={`w-4 h-4 mr-1 ${
                        i < t.rating
                          ? "text-accentColor dark:text-yellow-400"
                          : "text-gray-600"
                      }`}
                    />
                  ))}
              ...
              {/* Feedback bubble */}
              <p className="text-primaryText mb-4 leading-relaxed">
                {t.feedback}
              </p>

              {/* Message tail */}
              <div className="absolute top-5 -left-2 w-0 h-0 border-t-8 border-b-8 border-r-8 border-t-transparent border-b-transparent border-r-accentColor/20 dark:border-r-gray-800"></div>
